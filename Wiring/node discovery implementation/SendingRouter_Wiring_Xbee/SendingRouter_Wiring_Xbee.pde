...
void printActiveEndpoints(const zdo_active_ep_rsp_header_t *rsp) {
  Serial.println("Active endpoints response");

  Serial.print("  About: 0x");
  if ((uint8_t)(rsp->network_addr_le >> 8) < 0x10)
    Serial.write('0');
  Serial.print((uint8_t)(rsp->network_addr_le >> 8), HEX);
  if ((uint8_t)rsp->network_addr_le < 0x10)
    Serial.write('0');
  Serial.print((uint8_t)rsp->network_addr_le, HEX);

  Serial.println();

  Serial.print("  Endpoints found: 0x");
  printHex(Serial, rsp->endpoints, rsp->ep_count, ", 0x", NULL);
  Serial.println();
}

void printHex(Print& p, const uint8_t* buf, size_t len, 
              const char*[] byte_sep, const char*[] group_sep, 
              size_t group_by) {
  size_t cur_group = 0;
  while (len--) {
